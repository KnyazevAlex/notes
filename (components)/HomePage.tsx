...
import NoteDisplay from "@/(components)/loadedNotesForMobile";
import Card from "./Card";
import Link from "next/link";
import {motion} from 'motion/react'


export default function MobilePage({mobileNotes} : any) {

  return (
    <div className="min-h-screen h-100vh bg-[#0F172A] text-slate-100 p-6 overflow-">
      <div className="max-w-5xl mx-auto space-y-8">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          >
            ...
            <div className="lg:hidden bg-linear-to-br from-indigo-600 to-indigo-700 rounded-xl p-6 shadow-md text-white ">
                 <div className="loadedNotesWrap grid grid-cols-3 gap-6">
            {
                ...
                ?
                 ...
                 :
                mobileNotes.map((note : any) => (
                    <Card
                    title={note.title.length > 10 ? `${(note.title).slice(0,10)}.....` : note.title}
                    subtitle={note.subtitle.length > 10 ? `${(note.subtitle).slice(0,10)}.....` : note.subtitle}
                    content={note.body}
                    key={note.id}
                    id={note.id}
